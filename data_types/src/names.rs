...
pub fn org_and_bucket_to_database<'a, O: AsRef<str>, B: AsRef<str>>(
    ...
    let non_encode_chars = NON_ALPHANUMERIC;

    let org: Cow<'_, str> = percent_encode(org.as_ref().as_bytes(), non_encode_chars).into();
    let bucket: Cow<'_, str> = percent_encode(bucket.as_ref().as_bytes(), non_encode_chars).into();
